...
public class owl2 : MonoBehaviour {
	...
	void Start () {
		...
		oriColor = GameObject.Find ("nesttop3").GetComponent<Renderer> ().material.color ;
		oriNest = GameObject.Find ("nesttop3").GetComponent<Renderer> ().material.color;
		isFighting = false;
		producer = GameObject.Find ("MouseProducer");
		fightPro = GameObject.Find ("CloudProducer");
		scorePad = GameObject.Find ("score-3");
		isCarrying = false;
		score = 1;
		gotStolen = false;
...
//			}
			...
			owl getpressure2 = owl1.GetComponent<owl> ();
			float pressure2 = getpressure2.pressure2;
			owl getPotB = owl1.GetComponent<owl> ();
			float potB = getPotB.potB;
			Vector3 destination = new Vector3 (0, potB, 0);
			...
			Debug.LogError ("Hooty's score: " + score);
			...
			if (isCarrying == true) {
				GameObject.Find ("owlbody2").GetComponent<Renderer> ().material.color = new Color (Mathf.Sin (Time.time * 10.0f) / 4.0f + 0.5f, Mathf.Sin (Time.time * 10.0f) / 4.0f + 0.5f, 0.0f);
			} else {
				GameObject.Find ("owlbody2").GetComponent<Renderer> ().material.color = new Color (1.0f, 1.0f, 0.0f) ;
			}
			if (isOut && score > 0) {
				GameObject.Find ("nesttop3").GetComponent<Renderer> ().material.color = new Color (Mathf.Sin (Time.time * 10.0f) / 4.0f + 0.5f, Mathf.Sin (Time.time * 10.0f) / 4.0f + 0.5f, 0.0f);
			} else {
				GameObject.Find ("nesttop3").GetComponent<Renderer> ().material.color = oriNest;
			}
		} else if (state == 0) {
			rotateSpeed = 1.0f;
			moveSpeed = 0.0f;
	...
	void OnTriggerEnter(Collider other){
		...
		if (isCarrying == false) {
			if (other.gameObject.name == "nest") {
				...
				gameObject.transform.Rotate (new Vector3 (0.0f, 180.0f, 0.0f));
			} else if (other.gameObject.name == "nest2") {
